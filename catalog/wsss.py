...
def deleteWatchCategory(w1xid):
    ...
    if request.method == "POST":
        ...
        return render_template(
            'deleteWatchCategory.html', w1x=w1x, w1x_c12=w1x_c12)
...
def categoryItemsJSON(watch_name):
    ...
    watches = session.query(Watchlist).filter_by(watchname=watchCategory).all()
...
def ItemJSON(watch_name, edition_name):
    ...
    watchEdition = session.query(Watchlist).filter_by(
           modelname=edition_name, watchname=watchCategory).one()
